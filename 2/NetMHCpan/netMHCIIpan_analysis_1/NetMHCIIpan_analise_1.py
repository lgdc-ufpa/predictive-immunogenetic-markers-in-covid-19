...
# alelos_path = os.getcwd() + '/alelos/alelostestefinal.xlsx'
alelos_path = os.getcwd() + '/alelos/'
# book_alelos = load_workbook(alelos_path)
# sheet = book_alelos.active
book_alelos = None
sheet = None

#Tenta abrir o arquivo dos alelos
abrir = False
print('-- > Alelos em ' + alelos_path)
for elem in os.listdir(alelos_path):
    print(elem)

while not abrir:
    try:
        nome_alelos = str(input("\nEntre com o nome do arquivo de alelos sem a extensão .xlsx:"))
        # epitopos_txt = open(alelos_path + nome_alelos+'.xlsx', "r")
        book_alelos = load_workbook(alelos_path + nome_alelos + '.xlsx')
        sheet = book_alelos.active
        abrir = True

    except:
        print("Nome incorreto. Insira o nome sem a extensão .xlsx")

epitopos_path = str
epitopos_txt = str
# epitopos_path = "C:\\Users\\mauro\\OneDrive\\Doutorado\\Epitopos\\"
epitopos_path = os.getcwd() + '/epitopos/'
epitopos = ""
...
abrir = False
print('\n--> Epitodos em ' + epitopos_path)
for elem in os.listdir(epitopos_path):
    print(elem)

while not abrir:
    try:
        nome_epitopos = str(input("\nEntre com o nome do epitopo sem a extensão .txt:"))
...
while(ultima_analise != numero_analises):
    ...
    if (not final):
        ...
        alelos.append(sheet["A" + str(ultima_analise + 1)].value)
        print(alelos)
    ...
    SalvarEstado.salvarEstado(diretorio, nome_epitopos, numero_analises, ultima_analise, n_alelos_remanescentes=0)
# else:
    # # netMHCpan
